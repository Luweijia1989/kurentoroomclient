...
QSharedPointer<QWebRTCSessionDescription> QWebRTCPeerConnection::local_description()
{
	if (!m_impl->_conn->local_description())
		return QSharedPointer<QWebRTCSessionDescription>();

	return QSharedPointer<QWebRTCSessionDescription_impl>(new QWebRTCSessionDescription_impl(m_impl->_conn->local_description()));
}

QSharedPointer<QWebRTCSessionDescription> QWebRTCPeerConnection::remote_description()
{
	if (!m_impl->_conn->remote_description())
		return QSharedPointer<QWebRTCSessionDescription>();

	return QSharedPointer<QWebRTCSessionDescription_impl>(new QWebRTCSessionDescription_impl(m_impl->_conn->remote_description()));
}
